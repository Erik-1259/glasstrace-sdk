...
- d076c34: Harden the middleware and async-context wrappers. A per-call watchdog now
...
- 9408864: Add default-deny bounded-row result capture for Prisma `findMany`. When
...
- 3863e86: Attach the aggregate-result family marker last, as its own ordered write
...
- 92c88e3: Add explicit, default-deny Prisma `count` / `aggregate` result capture. A
...
- 8770ea7: Consume the server-derived result-evidence capability envelope through the
...
- 8519e7e: Teach installed agent guidance to let `find_trace_candidates` use its
...
- eab04d7: Clarify in the README that emitting a value-fidelity scalar is separate from
...
- 4fb1dbe: Export `BOUNDARY_MASKED_SCOPE_VALUES` and the `BoundaryMaskedScope` type from
...
- 18198a6: Add a Node-only `@glasstrace/sdk/diagnostics` subpath: a flag-gated,
...
- ed479bd: Teach installed agent guidance to bind trace evidence to source edit boundaries before editing.
...
- 83ecfb4: Relabel SDK-emitted scalar budget overflow omissions as
...
- 5a29423: Harden the request-middleware and async-causality wrappers:
