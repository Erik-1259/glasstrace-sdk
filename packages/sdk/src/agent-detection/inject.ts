 ...
 * legacy unstamped markers and version-stamped markers.
 *
 * Two shapes:
 *   - Legacy: `<!-- glasstrace:mcp:start -->`
 *   - Stamped: `<!-- glasstrace:mcp:start v=1.4.0 -->`
 *
 * The optional `v=<semver>` capture group is the version stamp.
 * Recognising the legacy form is
 * load-bearing for the backward-compatibility constraint: an
 ...
 *
 * Accepts both legacy unstamped markers and version-stamped
 * markers. Returns `null` if the line is not a start
 ...
 *
 * Recognises both the legacy unstamped marker form and
 * the version-stamped form for both HTML-comment and hash-prefix
 ...
 * MOST RECENT start preceding the end. This matches the legacy
 ...
 * and version-stamped markers, so an upgrading user's first
 * re-render replaces the existing block in place rather than
 * appending a duplicate (the backward-compatibility
 ...
 * unstamped markers and version-stamped markers. Used by the
