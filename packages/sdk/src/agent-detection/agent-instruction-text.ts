...
export function buildAgentInstructionBody(): string {
  return [
    ...
    "   **Complete-request preference:** Choose the next action under the existing routing, evidence, authorization, stop, and follow-up-budget rules before considering `nextRequests`. For a selected `find_trace_candidates` search, prefer a single unambiguous matching entry whose `tool` is exactly `find_trace_candidates`, whose `intent` is the selected `continue`, `replace_locator`, `widen_window`, `drop_locator`, or `include_framework_internal` action, and whose complete destination-valid `args` include an explicit valid `timeWindow` and `limit`. The entry must preserve the search scope except for the selected change and obey the window/cursor and retention guards below. Copy its `tool` and `args` exactly; do not merge legacy fields or reconstruct arguments. If `nextRequests` is absent, malformed, ambiguous, or does not match the chosen action, use the guarded legacy options below; if those are not valid, stop rather than guess. The guards' manual argument reconstruction is only the legacy fallback. `nextRequests` does not choose an action, increase a budget, authorize a credential or permission change, or turn source-inspection advice into an executable call.",
