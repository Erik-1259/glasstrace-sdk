...
describe("BOUNDARY_MASKED_SCOPE_VALUES", () => {
  it("pins the wire value set to exactly these members", () => {
    ...
    // producer and consumer. `toEqual` is an exact match, so it also pins the
    // set to EXACTLY these two members — and because BoundaryMaskedScope is
    // derived as `(typeof BOUNDARY_MASKED_SCOPE_VALUES)[number]`, the union
    // cannot widen without failing here.
